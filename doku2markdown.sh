...
find ./ -type f  -name "*.md"| xargs dos2unix  > /dev/null

# 转换标题
find ./ -type f  -name "*.md" | xargs sed -i -e 's/^======/#/' -e 's/======$//'
find ./ -type f  -name "*.md" | xargs sed -i -e 's/^=====/##/' -e 's/=====$//'
find ./ -type f  -name "*.md" | xargs sed -i -e 's/^====/###/' -e 's/====$//'
find ./ -type f  -name "*.md" | xargs sed -i -e 's/^===/####/' -e 's/===$//'
find ./ -type f  -name "*.md" | xargs sed -i -e 's/^==/#####/' -e 's/==$//'
...
# 转换代码
find ./ -type f  -name "*.md" |xargs sed -i 's/<sxh \([a-zA-Z0-9]*\).*/\n```\1/'
find ./ -type f  -name "*.md" | xargs sed -i 's/<sxh>/\n```bash/' 
find ./ -type f  -name "*.md" | xargs sed -i 's/<\/sxh>/```\n/' 
